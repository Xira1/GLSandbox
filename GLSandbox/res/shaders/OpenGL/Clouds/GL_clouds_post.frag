...
vec4 gaussianBlur(sampler2D tex, vec2 uv) {
    ...
    float kernel[9] = float[](
        1.0 / 16.0, 2.0 / 16, 1.0 / 16,  
        1.0 / 16.0, 4.0 / 16, 2.0 / 16,  
        1.0 / 16.0, 2.0 / 16, 1.0 / 16  
    );
    ...
    for (int i = 0; i < 9; i++) {
        vec4 pixel = texture(tex, uv.st + offset[i]);
        sampleTex[i] = pixel;
    }
...
void main() {
    ...
    bvec2 upperLimit = lessThan(lightPos.xy, vec2(1.0));
	...
	vec3 colorWithRays = FragColor.rgb +  (smoothstep(0.0, 1.0, colRays) * exposure - 0.2);
